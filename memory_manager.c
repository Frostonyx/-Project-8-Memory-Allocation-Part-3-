...
void myfree(void* ptr) {
    struct block* curr = (struct block*)ptr - 1; // get the block pointer from user pointer
    curr->in_use = 0; // mark the block as not in use

    // merge contiguous free blocks
    struct block* curr_free = head;
    while (curr_free && curr_free->next) {
        if (!curr_free->in_use && !curr_free->next->in_use) { // merge two contiguous free blocks
            size_t new_size = curr_free->size + GET_PADDING(curr_free) + curr_free->next->size + GET_PADDING(curr_free->next);
            curr_free->size = new_size;
            curr_free->next = curr_free->next->next;
        } else {
